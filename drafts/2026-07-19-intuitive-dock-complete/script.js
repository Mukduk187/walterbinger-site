...
function activeRoots() {
  ...
  if (worldNodeId) roots.add(worldNodeId);
  return roots;
}
...
function focusedNodeId() {
  return hoverNodeId || selectedNodeId || [...pinnedNodes][0] || null;
}

function applyScaleLayout(rootId, positions) {
  ...
  const slots = [
    [rootId, { x: 910, y: 382 }],
    [related[0], { x: 355, y: 455 }],
    [related[1], { x: 265, y: 286 }],
    [related[2], { x: 610, y: 584 }],
  ];
...
function currentPositions() {
  ...
  const center = nodesById.get(worldNodeId);
  const active = [...activeLenses];
  const related = new Set();
  active.forEach((lensId) => linksFor(worldNodeId, lensId).forEach((id) => related.add(id)));

  const positions = new Map();
  positions.set(worldNodeId, { x: 600, y: 310 });
  ...
  others.forEach((node, index) => {
    ...
    const radius = near ? 190 : 325;
    const angle = seed + index * ((Math.PI * 2) / others.length);
    const wobble = near ? 28 : 52;
...
function renderStars(positions, relatedIds) {
  ...
  nodes.forEach((node) => {
    ...
    const star = el("g", {
      ...
      tabindex: "0",
      role: "button",
      "aria-label": `${node.label}: single click to pin, double click to enter`,
      "data-node": node.id,
      transform: `translate(${pos.x} ${pos.y})`,
    });

    star.append(circle(0, 0, 34, "hotspot"));
    star.append(circle(0, 0, isPinned ? 4.8 : 3.4, "star-core"));
...
function actualStarPath(rootId, relatedIds, positions, shape) {
  ...
  if (shape === "scale") {
    ...
    const leftPan = positions.get(relatedIds[0]);
    const leftHook = positions.get(relatedIds[1]);
    const base = positions.get(relatedIds[2]);
    if (!root || !leftPan || !leftHook || !base) return "";
    const midX = (leftHook.x + root.x) / 2;
    const midY = (leftHook.y + root.y) / 2;
    return [
      `M${leftHook.x} ${leftHook.y} L${root.x} ${root.y}`,
      `M${leftHook.x} ${leftHook.y} L${leftPan.x} ${leftPan.y}`,
      `M${midX} ${midY} L${base.x} ${base.y}`,
    ].join(" ");
  }
...
function togglePin(id) {
  ...
  if (pinnedNodes.has(id)) {
    ...
    if (!pinnedNodes.size && !hoverNodeId) selectedNodeId = null;
  } else {
    pinnedNodes.add(id);
  }
  render();
...
function renderBodyState(nodeId) {
  ...
  document.body.classList.toggle("has-pins", pinnedNodes.size > 0);
