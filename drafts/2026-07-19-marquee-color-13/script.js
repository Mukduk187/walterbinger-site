...
function satelliteRadius(x = 600, index = 0) {
  ...
  return (nearEdge ? 88 : 112) + index * 8;
}
...
function renderStars(positions, relatedIds) {
  ...
  nodes.forEach((node) => {
    ...
    spectrum.slice(0, 7).forEach((item, index) => {
      ...
      bloom.style.setProperty("--bloom-opacity", (0.09 + item.strength * 0.21).toFixed(3));
      ...
      ring.style.setProperty("--ring-width", `${(1 + item.strength * 1.8).toFixed(2)}px`);
      ring.style.setProperty("--ring-opacity", (0.18 + item.strength * 0.36).toFixed(3));
    ...
    spectrum.slice(0, 7).forEach((item, index) => {
      ...
      dash.style.setProperty("--dash-opacity", clamp(0.42 + item.strength * 0.48).toFixed(3));
      dash.style.setProperty("--dash-offset", `${index * 13}px`);
      dash.style.setProperty("--dash-speed", `${(3600 + index * 320).toFixed(0)}ms`);
...
function renderObjectPreviews(positions) {
  ...
  ids.forEach((id) => {
    ...
    artifacts.forEach((artifact, index) => {
      ...
      const radius = satelliteRadius(pos.x, index);
      ...
      frame.style.setProperty("--satellite-orbit-duration", `${(12800 + index * 900).toFixed(0)}ms`);
      const img = htmlEl("img", { src: artifact.src, alt: artifact.title, loading: "lazy" });
      frame.append(img);
