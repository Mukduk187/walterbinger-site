...
function applyScaleLayout(rootId, positions) {
  ...
  const slots = [
    [rootId, { x: 870, y: 438 }],
    [related[0], { x: 350, y: 438 }],
    [related[1], { x: 610, y: 300 }],
    [related[2], { x: 610, y: 582 }],
  ];
...
function actualStarPath(rootId, relatedIds, positions, shape) {
  ...
  if (shape === "scale") {
    ...
    const pivot = positions.get(relatedIds[1]);
    const base = positions.get(relatedIds[2]);
    if (!root || !left || !pivot || !base) return "";
    const leftPanY = left.y + 76;
    const rightPanY = root.y + 76;
    return [
      `M${left.x} ${left.y} Q${pivot.x} ${pivot.y - 36} ${root.x} ${root.y}`,
      ...
      `M${root.x} ${root.y} L${root.x} ${rightPanY}`,
      `M${root.x - 118} ${rightPanY} C${root.x - 70} ${rightPanY + 54} ${root.x + 70} ${rightPanY + 54} ${root.x + 118} ${rightPanY}`,
