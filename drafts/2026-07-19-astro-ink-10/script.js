...
function spectrumGlow(spectrum) {
  if (!spectrum.length) return "none";
  return spectrum
    .map((item, index) => {
      const blur = Math.round(6 + item.strength * 18 + index * 2);
      const alpha = 0.22 + item.strength * 0.4;
      return `drop-shadow(0 0 ${blur}px ${rgba(item.color, alpha)})`;
    })
    .join(" ");
}
...
function applySpectrumStyles(element, spectrum) {
  ...
  element.style.setProperty("--spectrum-glow", spectrumGlow(spectrum));
}
...
function renderStars(positions, relatedIds) {
  ...
  nodes.forEach((node) => {
    ...
    spectrum.slice(0, 7).forEach((item, index) => {
      const bloom = ellipse(0, 0, 30 + index * 5 + item.strength * 7, 18 + index * 3 + item.strength * 4, "spectral-bloom");
      bloom.style.setProperty("--bloom-color", item.color);
      bloom.style.setProperty("--bloom-opacity", (0.06 + item.strength * 0.13).toFixed(3));
      bloom.style.setProperty("--bloom-turn", `${index * 27 - 18}deg`);
      aura.append(bloom);

      const ring = ellipse(0, 0, 28 + index * 6 + item.strength * 6, 18 + index * 4 + item.strength * 5, "spectral-ring");
      ring.style.setProperty("--ring-color", item.color);
      ring.style.setProperty("--ring-strength", item.strength.toFixed(3));
      ring.style.setProperty("--ring-width", `${(0.7 + item.strength * 1.2).toFixed(2)}px`);
      ring.style.setProperty("--ring-opacity", (0.1 + item.strength * 0.22).toFixed(3));
      ring.style.setProperty("--ring-turn", `${index * 29 - 15}deg`);
      aura.append(ring);
    });
    star.append(aura);
    ...
    spectrum.slice(0, 7).forEach((item, index) => {
      ...
      dash.style.setProperty("--dash-opacity", (0.2 + item.strength * 0.58).toFixed(3));
      dash.style.setProperty("--dash-offset", `${index * -10}px`);
      dash.style.setProperty("--dash-speed", `${(1380 + index * 115).toFixed(0)}ms`);
      addGlyph(dash, node.glyph);
      dashwork.append(dash);
    });
    star.append(dashwork);
    const inkKnot = el("g", { class: "ink-knot", "aria-hidden": "true" });
    addInkKnot(inkKnot);
    star.append(inkKnot);
...
function renderObjectPreviews(positions) {
  ...
  ids.forEach((id) => {
    ...
    const artifacts = artifactsFor(id).slice(0, 1);
    ...
    artifacts.forEach((artifact, index) => {
      const angle = satelliteAngle(index, artifacts.length, pos.x);
      const radius = 108 + index * 8;
      const frame = htmlEl("span", {
        class: `object-preview-frame frame-${index + 1}`,
        title: artifact.title,
      });
      frame.style.setProperty("--satellite-angle", `${angle}deg`);
      frame.style.setProperty("--satellite-counter-angle", `${-angle}deg`);
      frame.style.setProperty("--satellite-radius", `${radius}px`);
      frame.style.setProperty("--satellite-delay", `${index * 70}ms`);
      const img = htmlEl("img", { src: artifact.src, alt: artifact.title, loading: "lazy" });
      frame.append(img);
