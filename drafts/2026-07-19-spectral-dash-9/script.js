...
function renderStars(positions, relatedIds) {
  ...
  nodes.forEach((node) => {
    ...
    spectrum.slice(0, 7).forEach((item, index) => {
      const ring = circle(0, 0, 24 + index * 7 + item.strength * 6, "spectral-ring");
      ring.style.setProperty("--ring-color", item.color);
      ring.style.setProperty("--ring-strength", item.strength.toFixed(3));
      ring.style.setProperty("--ring-width", `${(1.2 + item.strength * 2.4).toFixed(2)}px`);
      ring.style.setProperty("--ring-opacity", (0.2 + item.strength * 0.68).toFixed(3));
      ring.style.setProperty("--ring-turn", `${index * 21 - 12}deg`);
      aura.append(ring);
    });
    star.append(aura);
