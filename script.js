...
function frequencyAttraction(nodeId, lensId) {
  const node = nodesById.get(nodeId);
  return clamp(Number(node?.composition?.[lensId] || 0));
}

function centralAttractionProfile(nodeId, lensIds = activeLensIds()) {
  if (!lensIds.length) return { mean: 0, total: 0, weights: [] };
  const weights = lensIds.map((lensId) => ({ lensId, weight: frequencyAttraction(nodeId, lensId) }));
  const total = weights.reduce((sum, item) => sum + item.weight, 0);
  return {
    mean: total / lensIds.length,
    total,
    weights,
  };
}
...
function applyRelationalLayout(positions, entries, center, originId, lensIds) {
  const points = entries.map((entry) => ({
    ...
    attraction: centralAttractionProfile(entry.node.id, lensIds).mean,
    vx: 0,
    vy: 0,
  }));
  ...
  for (let iteration = 0; iteration < 110; iteration += 1) {
    ...
    points.forEach((point) => {
      ...
      const centerDx = center.x - point.x;
      const centerDy = center.y - point.y;
      const centerDistance = Math.max(1, Math.hypot(centerDx, centerDy));
      const targetRadius = 155 + (1 - point.attraction) * 340;
      const radialForce = (centerDistance - targetRadius) * 0.014;
      point.vx += (centerDx / centerDistance) * radialForce + (point.anchorX - point.x) * 0.001;
      point.vy += (centerDy / centerDistance) * radialForce + (point.anchorY - point.y) * 0.001;
      point.vx *= 0.7;
      point.vy *= 0.7;
...
function applyLensLayout(positions, originId = null) {
  ...
  const entries = nodes
    ...
    .map((node) => {
      ...
      return {
        ...
        attraction: centralAttractionProfile(node.id, lensIds),
      };
    })
  ...
  if (lensIds.length === 1) {
    entries.forEach((entry) => {
      const radius = 225 + (1 - entry.attraction.mean) * 250;
  ...
  entries.forEach((entry, index) => {
    ...
    const radius = 265 + (1 - entry.attraction.mean) * 150 + (1 - entry.profile.coverage) * 35;
    positions.set(entry.node.id, {
...
function renderStars(positions, relatedIds, animatePositions = false) {
  ...
  nodes.forEach((node, index) => {
    ...
    const centralAttraction = centralAttractionProfile(node.id).mean;
    ...
    const star = el("g", {
      ...
      "data-node": node.id,
      "data-central-attraction": centralAttraction.toFixed(3),
